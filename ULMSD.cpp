...
//removing extra courses
void check(string stdCourseList[][100], string codelist[])
{
    int flag = 1;
    for (int i = 0; i < counter2; i++)
    {
        ...
        for (int j = 0; j < counter3[i]; j++)
        {
            ...
            for (int d = 0; d < counter; d++)
            {
                ...
                if (stdCourseList[i][j] == codelist[d])
                {
            ...
            if (flag == 1)
            {
                ...
                for (int c = j; c < counter3[j]; c++)
                {

                    stdCourseList[i][c] = stdCourseList[i][c + 1];
                }
                counter3[j]--;
            }
        }
...
int main()
{
    ...
    while (opp == 1)
    {
        ...
        else if (jadugar == 2)
        {
            ...
            if (flag == 1)
            {
                chachu = stdNamesList[global];
                size1 = chachu.size();
                if (password.size() == 3)
                {
                    if (chachu[size1 - 2] == password[1])
                    {
                        if (chachu[size1 - 1] == password[2])
                        {
                            if (chachu[size1] == password[3])
                            {
                                if (cin.fail())
                                {
                                    cin.clear();
